...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a HTMLCOIN address"));
    ...
    QRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/qrctoken"), tr("&HRC Tokens"), this);
    QRCTokenAction->setStatusTip(tr("HRC Tokens (send, receive or add Tokens in list)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your HTMLCOIN addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified HTMLCOIN addresses"));
    ...
    openAction->setStatusTip(tr("Open a htmlcoin: URI or payment request"));//QTUM_LINE

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible HTMLCOIN command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to HTMLCOIN network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("HTMLCOIN"); // default title
