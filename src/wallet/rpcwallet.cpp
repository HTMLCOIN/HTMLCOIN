...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw runtime_error(
            ...
            + HelpExampleCli("sendtoaddress", "\"HKQf7YmvMLEWvSoypQPnSEBn7LcxaPf7CH\", 0.1, \"donation\", \"seans outpost\", false, \"HtHbgQtE4J2XXDofytzvn3nXVZFvmuXzBo\", true")
            + HelpExampleRpc("sendtoaddress", "\"HKQf7YmvMLEWvSoypQPnSEBn7LcxaPf7CH\", 0.1, \"donation\", \"seans outpost\"")
            + HelpExampleRpc("sendtoaddress", "\"HKQf7YmvMLEWvSoypQPnSEBn7LcxaPf7CH\", 0.1, \"donation\", \"seans outpost\", false, \"HtHbgQtE4J2XXDofytzvn3nXVZFvmuXzBo\", true")
        );
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid HTMLCOIN address: ")+name_);
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (pwalletMain->IsCrypted() && (request.fHelp || request.params.size() < 2 || request.params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending HTMLCOIN\n"
            "\nArguments:\n"
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid HTMLCOIN address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    if (request.params.size() > 1) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid HTMLCOIN address");
